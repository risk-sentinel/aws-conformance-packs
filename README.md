...
<!-- CONTROL-SUMMARY:START -->

The packs crosswalk to **70 distinct Rev 5 controls**. Against the published baselines that is:
...
**Those percentages are a ceiling on ambition, not a score.** A control is counted here if any rule crosswalks to it — see the strength column below, and read `Reading these numbers honestly` before quoting any of it.

### By control family

| Family | In Moderate | Touched | Strongest claim | Where |
| --- | ---: | ---: | --- | --- |
| **AC** Access Control | 39 | 14 | full | CRYPTO, IAM, LOG, NET |
| **SC** System and Communications Protection | 25 | 14 | full | CRYPTO, NET, VCM |
| **AU** Audit and Accountability | 16 | 13 | full | LOG |
| **SI** System and Information Integrity | 18 | 8 | full | LOG, NET, RPL, VCM |
| **CM** Configuration Management | 24 | 7 | full | VCM |
| **IA** Identification and Authentication | 24 | 7 | full | CRYPTO, IAM, NET |
| **CP** Contingency Planning | 23 | 5 | full | RPL |
| **CA** Assessment, Authorization and Monitoring | 10 | 1 | partial | LOG |
| **RA** Risk Assessment | 10 | 1 | partial | VCM |
| **AT** Awareness and Training | 6 | 0 | — | GOV |
| **IR** Incident Response | 13 | 0 | — | GOV |
| **MA** Maintenance | 9 | 0 | — | GOV |
| **MP** Media Protection | 7 | 0 | — | GOV |
| **PE** Physical and Environmental Protection | 18 | 0 | — | GOV |
| **PL** Planning | 7 | 0 | — | GOV |
| **PS** Personnel Security | 9 | 0 | — | GOV |
| **SA** System and Services Acquisition | 17 | 0 | — | GOV |
| **SR** Supply Chain Risk Management | 12 | 0 | — | GOV |

**217 Moderate controls are untouched. They split two ways, and the split is the whole point.**

- **98** sit in the families marked `—` above — physical, personnel, training, planning, acquisition, incident response. These have no resource-configuration signal at all. AWS Config cannot see them, and no number of additional rules will change that. They belong to GOV, which evidences them against policy artifacts.
- **119** sit in families these packs already reach. That is the honest extension surface — controls where a Config rule could plausibly say something and none currently does. It is smaller than it looks: these catalogs already carry **every one of the 130 rules in AWS's own Rev 5 conformance pack**, plus 24 that pack does not ship. There is no backlog of obvious Rev 5 rules left unclaimed.

AWS's Rev 5 pack holds exactly 130 rules, which is the hard per-pack limit to the rule. It cannot grow without splitting, and a single pack at the cap is also a single blast radius, a single parameter budget and a single thing to redeploy. That is the argument for splitting by domain, and it is why these catalogs can hold more rules than AWS ships in one pack.

Treating those two numbers as one is what produces a coverage claim an assessor takes apart in the first hour.

AWS publishes 419 distinct managed rules in total, so roughly 267 remain unused — but those appear only in its PCI, C5, IRS-1075 and AI/ML packs, which carry no Rev 5 crosswalk. Adopting one means authoring the control mapping ourselves rather than inheriting it, which is why the number is a research backlog and not a to-do list.

**Strongest claim is the high-water mark for the family, not its average.** One `partial` rule in a family of thirty `supporting` ones puts `partial` in that cell. Per-control detail is in each pack's generated `coverage.md`.

**Moderate is a subset of High**, so every control touched in Moderate is also touched in High — the two rows report the same 70 controls against different denominators. Families with no baseline allocation at all (PM) are absent from the table rather than shown as zero.

<!-- CONTROL-SUMMARY:END -->

Generated by `tools/coverage_report.py`; CI fails if it drifts from the catalogs
and the vendored NIST index.
